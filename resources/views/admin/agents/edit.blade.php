...
@section('title', 'Agents / Edit Agent')

@section('content')
<div class="max-w-screen-xl mx-auto bg-[#121212] p-10 rounded-2xl shadow-xl border border-gray-800">
    <h1 class="text-3xl font-extrabold mb-8 text-[#4facfe] tracking-wide">EDIT AGENT</h1>

    <form action="{{ route('admin.agents.update', $agent->id) }}" method="POST" enctype="multipart/form-data" class="space-y-8">
        @csrf
        @method('PUT')
        ...
        <div>
            ...
            <input type="text" name="name" id="name"
                class="w-full bg-[#1a1a1a] text-white placeholder-gray-400 border border-gray-700 rounded-lg px-4 py-2.5 focus:ring-2 focus:ring-[#4facfe] focus:border-[#4facfe] transition"
                value="{{ old('name', $agent->name) }}" placeholder="Enter agent name" required>
            @error('name')
        ...
        <div>
            ...
            <input type="text" name="position" id="position"
                class="w-full bg-[#1a1a1a] text-white placeholder-gray-400 border border-gray-700 rounded-lg px-4 py-2.5 focus:ring-2 focus:ring-[#4facfe] focus:border-[#4facfe] transition"
                value="{{ old('position', $agent->position) }}" placeholder="Enter agent position" required>
            @error('position')
            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
        </div>
        ...
        {{-- Image Upload --}}
        <div class="border-2 border-dashed border-gray-600 rounded-xl p-8 text-center bg-[#1a1a1a] hover:border-[#4facfe]/60 transition">
            ...
            <div class="flex flex-col items-center gap-4">
                <label for="image" class="cursor-pointer flex flex-col items-center">
                    ...
                    <span class="mt-2 px-6 py-2 bg-gradient-to-r from-[#4facfe] to-[#00f2fe] text-white rounded-lg font-semibold hover:scale-105 transition-transform shadow-md">
                        Upload Image
                    </span>
                </label>
            </div>
            ...
            <div class="mt-6 text-center">
                ...
                <img src="{{ asset($agent->image) }}" alt="{{ $agent->name }}"
                    class="w-80 h-48 object-cover rounded-lg border border-gray-700 mx-auto">
            </div>
            @endif
        ...
        <div class="flex justify-between pt-6">
            ...
            <button type="submit"
                class="px-6 py-2 bg-gradient-to-r from-[#4facfe] to-[#00f2fe] text-white rounded-lg font-semibold hover:scale-105 shadow-lg transition">
                Update Agent
            </button>
        </div>
    </form>
