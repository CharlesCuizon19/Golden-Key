<div class="flex flex-col gap-10 py-20 mx-3 2xl:mx-20">
    ...
    <div class="grid grid-cols-1 gap-8 2xl:grid-cols-3">
        @foreach ($properties as $item)
            <a href="{{ route('unit-listing.singlepage', $item->id) }}" class="flex flex-col gap-3 cursor-pointer group">

                {{-- IMAGE --}}
                <div class="relative">
                    <div class="overflow-hidden rounded-lg">
                        <img src="{{ asset(optional($item->images->first())->image_path ?? 'images/default.jpg') }}"
                            class="w-full h-[20rem] transition duration-500 object-cover group-hover:scale-105">
                    </div>

                    {{-- STATUS --}}
                    <div
                        class="absolute rounded-lg top-3 left-3 px-4 py-2 text-black text-lg 2xl:text-lg
                            ...
                            @foreach ($item->features as $feature)
                                <div class="flex items-center gap-2">
                                    ...
                                    @if ($feature->pivot->quantity)
                                        <span
                                            class="text-sm font-bold 2xl:text-xl">({{ $feature->pivot->quantity }})</span>
                                    @endif
                                </div>
                            @endforeach
                        </div>
